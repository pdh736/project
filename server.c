...
*/
	...
	//서버 먼저 종료되더라도 다시 실행시 포트 바인드 에러 안나게 해줌
	...
	while(1) {
		...
		if(str_len > 0)
		{
			...
			client_info[clnt_cnt].index = clnt_cnt; 
			client_info[clnt_cnt].fd = clnt_sock; 
			...
			pthread_create(t_id+(clnt_cnt-1), NULL, handle_clnt, (void *)(client_info + (clnt_cnt-1)));	
			//make thread , pass client info void* type
			pthread_detach(t_id[clnt_cnt-1]);
...
void * handle_clnt(void *arg)
{
	...
	//obtain first clinet struct address for send all user	
